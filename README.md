...
3.2. Массив правил
3.3. Если нужно, можно создать свою функцию, которая выполнится после отправки, тогда ее тоже нужно будет передать как аргумент функции `validateForms`. 4. Также эта функция поддерживает работы с множественными чекбоксами/радиокнопками. Третьим параметром в функцию можно передать массив с настройками:
...
$options: ( parentClass: "cards", itemsClass: "cards__item", desktopGap: 30px,
desktopElems: 3, tablet: "1024px", tabletElems: 2, tabletGap: 30px, mobile:
"600px", mobileElems: 1, mobileGap: 20px ); @include flex-layout($options);
```
...
6. Немного обновлены базовые стили сборки:
   ...
   6.2. **box-sizing: border-box** задан глобально (без inherit), т.к. за все время использования сборки понял, что это лишено смысла.
   6.3. по умолчанию у .page добавлено свойство **scroll-behavior**.
7. Немного обновлены скрипты и модули:
   7.1. Изменился метод подключения **swiper**.
   7.2. Изменился метод подключения **Inputmask**.
...
Правила для stylelint.

1. `"extends": "stylelint-config-standard-scss",` - Этот параметр указывает, что ваша конфигурация использует стандартный набор правил для SCSS-файлов.
2. `"plugins": ["stylelint-order"],` - Этот параметр подключает плагин "stylelint-order", который позволяет контролировать порядок свойств в CSS.
3. `"rules": {` - Здесь начинается определение правил для stylelint.
4. `"order/order": ["custom-properties", "dollar-variables", "at-rules"],` - Это правило упорядочивает различные типы CSS-конструкций: сначала идут пользовательские свойства, затем переменные SCSS, и наконец, директивы @.
5. `"order/properties-alphabetical-order": null,` - Это правило отключает проверку на алфавитный порядок свойств.
6. `"order/properties-order": null,` - Это правило отключает проверку на определённый порядок свойств.
7. `"at-rule-empty-line-before": [...]` - Это правило управляет пустыми строками перед директивами @.
8. `"at-rule-no-unknown": null,` - Это правило отключает проверку на неизвестные директивы @.
9. `"at-rule-no-vendor-prefix": true,` - Это правило запрещает использование вендорных префиксов в директивах @.
10. `"declaration-no-important": true,` - Это правило запрещает использование !important в объявлениях свойств.
11. `"color-function-notation": null,` - Это правило отключает проверку на формат записи цветовых функций.
12. `"alpha-value-notation": null,` - Это правило отключает проверку на формат записи альфа-компонента цвета.
13. `"property-no-vendor-prefix": null,` - Это правило отключает проверку на использование вендорных префиксов в свойствах.
14. `"custom-property-pattern": null,` - Это правило отключает проверку на соответствие паттерну для пользовательских свойств.
15. `"selector-class-pattern": null,` - Это правило отключает проверку на соответствие паттерну для классов в селекторах.
16. `"selector-id-pattern": null,` - Это правило отключает проверку на соответствие паттерну для идентификаторов в селекторах.
17. `"color-named": null,` - Это правило отключает проверку на использование именованных цветов.
18. `"scss/double-slash-comment-inline": null,` - Это правило отключает проверку на однострочные комментарии с двумя слэшами.
19. `"scss/double-slash-comment-whitespace-inside": "always",` - Это правило требует наличие пробелов внутри однострочных комментариев с двумя слэшами.
20. `"scss/media-feature-value-dollar-variable": null,` - Это правило отключает проверку на использование переменных SCSS в значениях медиа-функций.
21. `"scss/operator-no-newline-after": null,` - Это правило отключает проверку на наличие новой строки после операторов SCSS.
22. `"scss/operator-no-newline-before": null,` - Это правило отключает проверку на наличие новой строки перед операторами SCSS.
23. `"scss/operator-no-unspaced": true,` - Это правило требует, чтобы операторы SCSS были разделены пробелами.
24. `"scss/partial-no-import": null,` - Это правило отключает проверку на импорт частичных файлов SCSS.
25. `"scss/percent-placeholder-pattern": null,` - Это правило отключает проверку на соответствие паттерну для плейсхолдеров SCSS.
26. `"scss/selector-no-redundant-nesting-selector": null,` - Это правило отключает проверку на избыточные вложенные селекторы SCSS.
27. `"scss/dollar-variable-no-missing-interpolation": null,` - Это правило отключает проверку на отсутствие интерполяции в переменных SCSS.
28. `"scss/dollar-variable-pattern": null,` - Это правило отключает проверку на соответствие паттерну для переменных SCSS.
29. `"scss/double-slash-comment-empty-line-before": null,` - Это правило отключает проверку на наличие пустой строки перед однострочными комментариями с двумя слэшами.
30. `"value-keyword-case": "lower",` - Это правило требует, чтобы значения свойств были записаны в нижнем регистре.
31. `"value-no-vendor-prefix": true,` - Это правило запрещает использование вендорных префиксов в значениях свойств.
32. `"scss/at-else-closing-brace-newline-after": "always-last-in-chain",` - Это правило требует наличия новой строки после закрывающей фигурной скобки директивы @else.
33. `"scss/at-else-closing-brace-space-after": "always-intermediate",` - Это правило требует наличия пробела после закрывающей фигурной скобки директивы @else.
34. `"scss/at-else-empty-line-before": "never",` - Это правило запрещает наличие пустой строки перед директивой @else.
35. `"scss/at-else-if-parentheses-space-before": "always",` - Это правило требует наличия пробела перед скобками директивы @if после @else.
36. `"scss/at-extend-no-missing-placeholder": null,` - Это правило отключает проверку на отсутствие плейсхолдера при использовании директивы @extend.
37. `"scss/at-function-parentheses-space-before": "never",` - Это правило запрещает наличие пробела перед скобками в директиве @function.
38. `"scss/at-function-pattern": null,` - Это правило отключает проверку на соответствие паттерну для названий функций SCSS.
39. `"scss/at-if-closing-brace-newline-after": "always-last-in-chain",` - Это правило требует наличия новой строки после закрывающей фигурной скобки директивы @if.
40. `"scss/at-if-closing-brace-space-after": "always-intermediate",` - Это правило требует наличия пробела после закрывающей фигурной скобки директивы @if.
41. `"scss/at-import-partial-extension-blacklist": null,` - Это правило отключает проверку на расширение импортируемых частичных файлов SCSS.
42. `"scss/at-mixin-argument
